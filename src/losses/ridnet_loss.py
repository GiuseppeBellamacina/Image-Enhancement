...
class CombinedLoss(nn.Module):
    ...
    def __init__(
        ...
        alpha=0.84,
        beta=0.16,
        ssim_value_eps: float = 1e-6,
        ssim_noise_std: float = 1e-7,
    ):
        super().__init__()
        self.alpha = alpha
        self.beta = beta
        self.ssim_value_eps = ssim_value_eps
        self.ssim_noise_std = ssim_noise_std
    ...
    def forward(self, pred, target):
        ...
        eps = self.ssim_value_eps
        pred_01 = pred_01.clamp(eps, 1.0 - eps)
        target_01 = target_01.clamp(eps, 1.0 - eps)
        if self.ssim_noise_std > 0.0:
            pred_01 = pred_01 + torch.randn_like(pred_01) * self.ssim_noise_std
            pred_01 = pred_01.clamp(eps, 1.0 - eps)
