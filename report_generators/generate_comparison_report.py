...
def main():
    ...
    report_lines.append(
        "| Experiment ID | Timestamp | LR_G | LR_D | λ_L1 | Patch | Epochs | PSNR | SSIM | MAE |"
    )
    report_lines.append(
        "|---------------|-----------|------|------|------|-------|--------|------|------|-----|"
    )
    ...
    for exp in experiments:
        ...
        report_lines.append(
            f"| {exp_id[:25]}{'...' if len(exp_id) > 25 else ''} | {timestamp} | {lr_g_str} | {lr_d_str} | {lambda_l1} | {patch_size} | {epochs} | {psnr_str} | {ssim_str} | {mae_str} |"
        )
    ...
    if experiments_with_metrics:
        ...
        report_lines.append(
            f"  - Learning Rates: G={best_psnr['lr_g']:.0e}, D={best_psnr['lr_d']:.0e}"
        )
        ...
        report_lines.append(
            f"  - Learning Rates: G={best_ssim['lr_g']:.0e}, D={best_ssim['lr_d']:.0e}"
        )
        ...
        report_lines.append(
            f"  - Learning Rates: G={best_mae['lr_g']:.0e}, D={best_mae['lr_d']:.0e}"
        )
        ...
        report_lines.append(
            f"- **PSNR**: Mean={statistics.mean(psnr_values):.2f} dB, Std={statistics.stdev(psnr_values) if len(psnr_values) > 1 else 0:.2f} dB, Range=[{min(psnr_values):.2f}, {max(psnr_values):.2f}]"
        )
        report_lines.append(
            f"- **SSIM**: Mean={statistics.mean(ssim_values):.3f}, Std={statistics.stdev(ssim_values) if len(ssim_values) > 1 else 0:.3f}, Range=[{min(ssim_values):.3f}, {max(ssim_values):.3f}]"
        )
        report_lines.append(
            f"- **MAE**: Mean={statistics.mean(mae_values):.4f}, Std={statistics.stdev(mae_values) if len(mae_values) > 1 else 0:.4f}, Range=[{min(mae_values):.4f}, {max(mae_values):.4f}]\n"
        )
    ...
    if experiments_with_metrics:
        ...
        report_lines.append(
            f"- Tested Generator LRs: {', '.join([f'{lr:.0e}' for lr in lr_g_unique])}"
        )
        ...
        for lr_g in lr_g_unique:
            ...
            report_lines.append(
                f"  - LR_G={lr_g:.0e}: Avg PSNR={avg_psnr:.2f} dB, Avg SSIM={avg_ssim:.3f} ({len(exps_with_lr)} experiments)"
            )
        ...
        report_lines.append(
            f"- Tested Lambda L1 values: {', '.join([str(int(lambda_val)) for lambda_val in lambda_unique])}"
        )

        for lambda_val in lambda_unique:
            exps_with_lambda = [
                e for e in experiments_with_metrics if e["lambda_l1"] == lambda_val
            ]
            avg_psnr = statistics.mean([e["psnr"] for e in exps_with_lambda])
            avg_ssim = statistics.mean([e["ssim"] for e in exps_with_lambda])
            report_lines.append(
                f"  - λ_L1={int(lambda_val)}: Avg PSNR={avg_psnr:.2f} dB, Avg SSIM={avg_ssim:.3f} ({len(exps_with_lambda)} experiments)"
            )
    ...
    if experiments_with_metrics:
        best_overall = max(
            experiments_with_metrics, key=lambda x: x["psnr"] + 10 * x["ssim"]
        )  # Weighted score
        report_lines.append("### Best Overall Configuration")
        report_lines.append("Based on weighted metrics (PSNR + 10×SSIM):")
        report_lines.append(f"- **Experiment**: {best_overall['id']}")
        report_lines.append(
            f"- **Results**: PSNR={best_overall['psnr']:.2f} dB, SSIM={best_overall['ssim']:.3f}, MAE={best_overall['mae']:.4f}"
        )
        report_lines.append(
            f"- **Config**: LR_G={best_overall['lr_g']:.0e}, LR_D={best_overall['lr_d']:.0e}, λ_L1={int(best_overall['lambda_l1'])}"
        )
        report_lines.append("")
    ...
    report_lines.append(
        "2. Lambda L1 parameter significantly affects reconstruction quality"
    )
    ...
    report_lines.append(
        "5. Consider ensemble methods or model averaging for best results"
    )
    ...
    print(
        f"📊 Analyzed {len(experiments)} experiments, {len(experiments_with_metrics)} with complete metrics"
    )
