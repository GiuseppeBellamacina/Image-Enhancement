...
def extract_timestamp(experiment_id: str) -> str:
    ...
    try:
        ...
        print(f"❌ Failed to extract timestamp from {experiment_id}: {e}")
    return "Unknown"
...
def create_experiment_summary(experiment_dir: Path) -> Dict[str, Any]:
    ...
    summary = {
        ...
        "configuration": {
            ...
            "degradation_parameters": {"noise_sigma": config.get("noise_sigma")},
            "training_setup": {
    ...
    if config.get("lambda_L1", 100) != 100:
        notes["key_observations"].append(
            f"Non-standard lambda_L1: {config.get('lambda_L1')}"
        )
    ...
    if config.get("scheduler"):
        notes["key_observations"].append(
            f"Learning rate scheduler: {config.get('scheduler')}"
        )

    if epochs_completed > 0:
        notes["key_observations"].append(
            f"Training stopped at epoch {epochs_completed}"
        )
