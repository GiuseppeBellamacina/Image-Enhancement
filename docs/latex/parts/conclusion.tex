...
\begin{enumerate}
    ...
    \item \textbf{Architectural advantages scale with degradation severity:} At $\sigma = 50$, NAFNet and the DAE achieve comparable performance ($\Delta = +0.05$~dB). At $\sigma = 100$, the gap widens to +2.09~dB, indicating that skip connections and attention mechanisms become essential only when noise exceeds the bottleneck's encoding capacity.
    
    \item \textbf{Model scaling shows diminishing returns:} Tripling NAFNet encoder depth yields only +0.08~dB while consuming 3.5$\times$ more memory, suggesting that training strategy innovations (multi-stage, loss scheduling) offer better returns than architectural scaling alone.
\end{enumerate}
