...
def main():
            ...
            if metrics_file.exists():
                ...
                if metrics:
                    all_evaluations.append(
                        {"experiment_id": exp_dir.name, "evaluation_metrics": metrics}
                    )
    ...
    md_lines.append(
        "| Experiment ID | N Images | PSNR (mean±std) | SSIM (mean±std) | MAE (mean±std) | MSE (mean±std) |"
    )
    md_lines.append(
        "|---------------|----------|-----------------|-----------------|----------------|----------------|"
    )
    ...
    for eval_data in all_evaluations:
        ...
        if "mean" in metrics:
            ...
            md_lines.append(
                f"| {exp_id[:30]}{'...' if len(exp_id) > 30 else ''} | {n_images} | {psnr_str} | {ssim_str} | {mae_str} | {mse_str} |"
            )
        ...
        if "mean" in metrics:
            ...
            if "per_image" in metrics:
                ...
                for img_metrics in metrics["per_image"]:
                    ...
                    md_lines.append(
                        f"| {filename} | {psnr:.2f} | {ssim:.4f} | {mae:.6f} | {mse:.6f} |"
                    )
    ...
    if valid_evals:
        # Best PSNR
        best_psnr = max(
            valid_evals, key=lambda x: x["evaluation_metrics"]["mean"]["psnr"]
        )
        md_lines.append("### Best PSNR")
        md_lines.append(f"- **Experiment**: {best_psnr['experiment_id']}")
        md_lines.append(
            f"- **PSNR**: {best_psnr['evaluation_metrics']['mean']['psnr']:.2f} dB"
        )
        md_lines.append(
            f"- **SSIM**: {best_psnr['evaluation_metrics']['mean']['ssim']:.4f}"
        )
        md_lines.append(
            f"- **MAE**: {best_psnr['evaluation_metrics']['mean']['mae']:.6f}\n"
        )

        # Best SSIM
        best_ssim = max(
            valid_evals, key=lambda x: x["evaluation_metrics"]["mean"]["ssim"]
        )
        md_lines.append("### Best SSIM")
        md_lines.append(f"- **Experiment**: {best_ssim['experiment_id']}")
        md_lines.append(
            f"- **SSIM**: {best_ssim['evaluation_metrics']['mean']['ssim']:.4f}"
        )
        md_lines.append(
            f"- **PSNR**: {best_ssim['evaluation_metrics']['mean']['psnr']:.2f} dB"
        )
        md_lines.append(
            f"- **MAE**: {best_ssim['evaluation_metrics']['mean']['mae']:.6f}\n"
        )

        # Best MAE (lowest)
        best_mae = min(
            valid_evals, key=lambda x: x["evaluation_metrics"]["mean"]["mae"]
        )
        md_lines.append("### Best MAE (Lowest Error)")
        md_lines.append(f"- **Experiment**: {best_mae['experiment_id']}")
        md_lines.append(
            f"- **MAE**: {best_mae['evaluation_metrics']['mean']['mae']:.6f}"
        )
        md_lines.append(
            f"- **PSNR**: {best_mae['evaluation_metrics']['mean']['psnr']:.2f} dB"
        )
        md_lines.append(
            f"- **SSIM**: {best_mae['evaluation_metrics']['mean']['ssim']:.4f}\n"
        )
    ...
    if valid_evals:
        print("\n🏆 Best Results:")
        print(
            f"   PSNR: {max(e['evaluation_metrics']['mean']['psnr'] for e in valid_evals):.2f} dB"
        )
        print(
            f"   SSIM: {max(e['evaluation_metrics']['mean']['ssim'] for e in valid_evals):.4f}"
        )
        print(
            f"   MAE:  {min(e['evaluation_metrics']['mean']['mae'] for e in valid_evals):.6f}"
        )
