...
class DenoisingAutoencoder(torch.nn.Module):
    def __init__(self, features=32):
        ...
        self.encoder = torch.nn.Sequential(
            # 128x128 -> 64x64
            torch.nn.Conv2d(
                3, features, kernel_size=3, stride=2, padding=1, bias=False
            ),
            torch.nn.BatchNorm2d(features),
            torch.nn.LeakyReLU(0.2, inplace=True),
            # 64x64 -> 32x32
            torch.nn.Conv2d(
                features, features * 2, kernel_size=3, stride=2, padding=1, bias=False
            ),
        ...
        self.decoder = torch.nn.Sequential(
            ...
            torch.nn.Upsample(scale_factor=2.0, mode="bilinear", align_corners=False),
            torch.nn.Conv2d(
                features * 2, features, kernel_size=3, stride=1, padding=1, bias=False
            ),
            torch.nn.BatchNorm2d(features),
            torch.nn.LeakyReLU(0.2, inplace=True),
            torch.nn.Upsample(scale_factor=2.0, mode="bilinear", align_corners=False),
            torch.nn.Conv2d(
                features, 3, kernel_size=3, stride=1, padding=1, bias=False
            ),
            torch.nn.Tanh(),  # Output between -1 and 1
        )
