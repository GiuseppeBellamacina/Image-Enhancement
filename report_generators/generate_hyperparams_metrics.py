...
def main():
    ...
    for exp_dir in sorted(experiments_base.iterdir()):
        if exp_dir.is_dir() and not exp_dir.name.startswith("."):
            ...
            experiments_data.append(
                {"hyperparameters": hyperparams, "final_metrics": metrics}
            )
    ...
    for exp_data in experiments_data:
        ...
        md_lines.append(
            f"- **Discriminator Features**: {hyper['discriminator_features']}"
        )
        ...
        if metrics:
            ...
            if "train_loss_G_GAN" in metrics:
                md_lines.append(
                    f"- **Train Loss G GAN** (adversarial): {metrics['train_loss_G_GAN']:.6f}"
                )
            if "train_loss_G_L1" in metrics:
                md_lines.append(
                    f"- **Train Loss G L1** (reconstruction): {metrics['train_loss_G_L1']:.6f}"
                )
