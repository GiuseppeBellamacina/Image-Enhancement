...
for cell in nb.cells:
    if cell.cell_type == 'code':
        # 1. Update imports
        if 'from src.losses.combined_loss import CombinedLoss' in cell.source:
            cell.source = cell.source.replace('from src.losses.combined_loss import CombinedLoss', 'from src.losses.combined_loss import get_criterion')
        
        # 2. Update config block
        if 'config = {' in cell.source and '"model_name": "nafnet"' in cell.source:
            cell.source = """config = {
    ...
    "seed": 42,
        ...
        # 3. Update model init
        if 'model = NAFNet(' in cell.source:
            cell.source = cell.source.replace(
                '    drop_out_rate=config["naf_drop_out_rate"],\n).to(config["device"]) ##',
                '    drop_out_rate=config["naf_drop_out_rate"],\n    drop_path_rate=config.get("naf_drop_path_rate", 0.0),\n).to(config["device"])'
            )
            # Remove exact old criterion string, replace with new one
            if 'criterion = CombinedLoss(alpha=config["loss_alpha"], beta=config["loss_beta"]).to(' in cell.source:
                # Instead of simple replace which might be multiline, reconstruct
                import re
                cell.source = re.sub(
        ...
        if 'load_pretrained_model(' in cell.source and 'FINE-TUNING' in cell.source:
...
with open(nb_path, 'w', encoding='utf-8') as f:
    nbformat.write(nb, f)
print('Notebook updating using nbformat finished!')
